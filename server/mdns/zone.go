...
type MDNSService struct {
	...
	IPs      GetAddrs // IP addresses for the service's host
...
type GetAddrs interface {
	GetAddress() (addrs []gonetworkmanager.IP4Address)
}
...
func NewMDNSService(instance, service, domain, hostName string, port int, getAddr GetAddrs, txt []string) (*MDNSService, error) {
	// Sanity check inputs
	if instance == "" {
